...
int main(){
    ...
    std::vector<unsigned char> image(0x1000000),world(0xc000),actors(5*0x3450),row(0xd0);
    ...
    put(base+0xdce5d0,w);put(w+0xa5a8,a);put(w+0xa5b0,a+actors.size());put(w+0xbf48,a);
    put(base+0xe02370,0);put(base+0xe05dc8,a);small_string(base+0xde8030,"OwnerAccount");small_string(a+0x6d8,"owneraccount");
    put(base+0xf984b4,2);put(w+0x190,0);put(w+0x28c,2);
    for(int i=0;i<5;++i){put(a+i*0x3450+0x78,i);put<unsigned char>(a+i*0x3450+0x81,1);put(a+i*0x3450+0x7c4,i==0?0:1);}
    put(a+0x3450+0x7c4,0);assert(ssc_names::identify(a).local);assert(!ssc_names::identify(a+0x3450).local);
    put(w+0xbf48,a+0x3450);assert(ssc_names::identify(a).local); // Spectating another human must not change own identity.
    small_string(a+0x3450+0x780,"OwnerAccount");assert(!ssc_names::identify(a+0x3450).local);
    small_string(a+0x6d8,"OtherAccount");assert(!ssc_names::identify(a).local);small_string(a+0x6d8,"OwnerAccount");put(a+0x3450+0x7c4,1);
    ...
    put(r+0x2c,0);ssc_names::cosmetics=true;put(base+0xf94db4,2.5f);put(base+0xf95810,.5f);invoke();assert(pp==.25f&&rr==6);
    put(a+0x3450+0x7c4,0);invoke();assert(pp==.25f);
    put(base+0xe02370,1);invoke();assert(pp==14);put(base+0xe02370,0);
    ...
    small_string(a+0x3450+0x780,"OtherPlayer");assert(ssc_names::unambiguous_event_account((uintptr_t)owned.data()));
    ...
    float phase=.75f;put(base+0xf94db4,3.4e38f);put(base+0xf95810,3.4e38f);assert(!ssc_names::native_phase(phase)&&phase==.75f);
