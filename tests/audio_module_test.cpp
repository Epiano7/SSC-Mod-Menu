...
int main(int argc,char** argv){
    assert(argc==2);auto root=std::filesystem::path(argv[1]);std::filesystem::create_directories(root/"game/data/sounds");
    ...
    ssc_audio::write(root/"game/data/sounds/duplicate.wav",reference);ssc_sound::initialize(root/"game",root/"state",true);assert(ssc_sound::replacements.empty());
    ...
    std::cout<<"PASS: normalization, clipping ceiling, format validation, buffer substitution, unknown passthrough, disable/restart, ambiguous audio and original preservation\n";
}
