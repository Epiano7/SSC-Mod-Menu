...
inline __attribute__((noinline)) void APIENTRY capture_call_list(GLuint list){
 if(reinterpret_cast<uintptr_t>(__builtin_return_address(0))==game_base+0x47a2c7)capture_cached_quad();
...
inline bool start_capture(){
 ...
 auto list=reinterpret_cast<CallListProc*>(game_base+0xdde670);
 ...
 *reinterpret_cast<CallListProc*>(game_base+0xdde670)=native_call_list;
...
inline void capture_queued_rectangle(const unsigned char* stack){
 ...
 if(game_base==reinterpret_cast<uintptr_t>(GetModuleHandleW(nullptr))){width=*reinterpret_cast<const int*>(game_base+0xf9f750);height=*reinterpret_cast<const int*>(game_base+0xf9f754);}
