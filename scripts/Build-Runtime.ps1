...
& $Compiler -O2 -std=c++17 -shared -static -Wall -Wextra -Werror -Wno-cast-function-type "$root/runtime/overlay.cpp" "$root/runtime/score_bridge.S" "$root/runtime/hud_bridge.S" -o "$build\runtime.dll" -lopengl32 -lgdi32 -lbcrypt -lcomdlg32 -lmfreadwrite -lmfplat -lmfuuid -lole32
if($LASTEXITCODE) {throw 'Runtime compilation failed'}
& $Compiler -O2 -std=c++17 -static -Wall -Wextra -Werror -Wno-cast-function-type "$root/tests/menu_hotkey_test.cpp" "$root/runtime/score_bridge.S" "$root/runtime/hud_bridge.S" -o "$build\menu_hotkey_test.exe" -lopengl32 -lgdi32 -lbcrypt -lcomdlg32 -lmfreadwrite -lmfplat -lmfuuid -lole32
if($LASTEXITCODE) {throw 'Hotkey test compilation failed'}
& "$build\menu_hotkey_test.exe"
if($LASTEXITCODE) {throw 'Hotkey tests failed'}
& $Compiler -O2 -std=c++17 -static -Wall -Wextra -Werror -Wno-cast-function-type "$root/tests/menu_ui_test.cpp" "$root/runtime/score_bridge.S" "$root/runtime/hud_bridge.S" -o "$build\menu_ui_test.exe" -lopengl32 -lgdi32 -lbcrypt -lcomdlg32 -lmfreadwrite -lmfplat -lmfuuid -lole32
...
& $Compiler -O2 -std=c++17 -static -Wall -Wextra -Werror "$root/tests/audio_module_test.cpp" -o "$build\audio_module_test.exe" -lbcrypt -lcomdlg32 -lmfreadwrite -lmfplat -lmfuuid -lole32
