...
Build-Runtime runs runtime tests. Build-Installer runs installer engine and UI workflow tests. Close Skillshot City first: the running-game guard also applies to installation fixtures. Test outputs stay under build/.
...
Native font/palette checks can additionally run `menu_ui_test.exe OUTPUT_DIRECTORY GAME_DIRECTORY GAME_EXE`. They read the owner's local game installation; do not commit its assets or executable.
